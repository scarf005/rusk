...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
            ...
            && let Some(mut hover) = rust_analyzer.hover(text, params).await
        {
            hover.range = hover
                .range
                .map(|range| rust_range_to_rusk_range(text, range));
...
fn rusk_position_to_rust_position(text: &str, position: Position) -> Option<Position> {
...
fn rust_range_to_rusk_range(text: &str, range: Range) -> Range {
    Range {
        start: rust_position_to_rusk_position(text, range.start),
        end: rust_position_to_rusk_position(text, range.end),
    }
}

fn rust_position_to_rusk_position(text: &str, position: Position) -> Position {
    let Ok(output) = transpile(text) else {
        return position;
    };
